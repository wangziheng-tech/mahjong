...
class FeatureAgent(MahjongGBAgent):
    ...
    def _extra_embedding_update(self):
        self.obs[self.OFFSET_OBS['SHOWN'] : ] = 0

        for tile, cnt in self.shownTiles.items():
            tile_id = self.OFFSET_TILE.get(tile)
            if tile_id is not None:
                self.obs[self.OFFSET_OBS['SHOWN'], tile_id] = min(cnt, 4)

        for player, rest in enumerate(self.tileWall):
            self.obs[self.OFFSET_OBS['WALL'], player] = max(rest, 0)

        if hasattr(self, 'curTile'):
            tile_id = self.OFFSET_TILE.get(self.curTile)
            if tile_id is not None:
                self.obs[self.OFFSET_OBS['CUR_TILE'], tile_id] = 1

        for player in range(4):
            for tile in self.history[player]:
                tile_id = self.OFFSET_TILE.get(tile)
                if tile_id is not None:
                    self.obs[self.OFFSET_OBS['HISTORY'] + player, tile_id] += 1

            for packType, tile, offer in self.packs[player]:
                layer = self.OFFSET_OBS['PACK'] + player
                if packType == 'CHI' and tile in self.OFFSET_TILE:
                    color = tile[0]
                    num = int(tile[1])
                    for k in range(-1, 2):
                        t = color + str(num + k)
                        tile_id = self.OFFSET_TILE.get(t)
                        if tile_id is not None:
                            self.obs[layer, tile_id] += 1
                else:
                    tile_id = self.OFFSET_TILE.get(tile)
                    if tile_id is not None:
                        self.obs[layer, tile_id] += 4 if packType == 'GANG' else 3

        self.obs[self.OFFSET_OBS['FLAGS'], 0] = 1 if self.wallLast else 0
        self.obs[self.OFFSET_OBS['FLAGS'], 1] = 1 if self.isAboutKong else 0
